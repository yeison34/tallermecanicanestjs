...
export class VehiculoService {
  ...
  async EncontrarPorPlaca(placa: string): Promise<Vehiculo | undefined> {
    return this.vehiculoRepo.findOne({ where: { placa } });
  }

  async InsertarVehiculo(vehiculo: VehiculoDto): Promise<Vehiculo> {
    const vehiculoExistente = await this.EncontrarPorPlaca(vehiculo.placa);
    if (vehiculoExistente) {
      throw new Error('Este vehículo ya está registrado.');
    }
